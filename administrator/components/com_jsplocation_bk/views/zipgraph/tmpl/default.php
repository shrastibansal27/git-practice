 ...
 * JSP Location extention prevents access to administration (back end)
...
$todaydate = date("Y-m-d");
 ...
 $dayhits[0][0] = "'".'Zip hits'."'";
 $dayhits[1][0] = "'".''."'";
 $db =& JFactory::getDbO();
$values =  "SELECT `zip`,sum(hits) as `sum`,`date` FROM `#__jsplocation_ziphits` WHERE `date` >=  '$date' GROUP BY `zip` ORDER BY `zip` desc";
$db->setQuery($values);
...
foreach($results as $items){
   ...
   $bridqry = "SELECT `zip`,sum(hits) as `sum`,`date` FROM `#__jsplocation_ziphits` WHERE `date` >=  '$date' GROUP BY `zip` ORDER BY `zip` desc";
$db->setQuery($bridqry);
...
<!-- start for today -->
	  ...
	  <div id="infodiv1" ><?php echo JText::_('DAILY_ZIP_DESC'); ?></div>
...
$qry2 = "SELECT date,SUM(hits) as total_hits,
            ...
            FROM `#__jsplocation_ziphits`  
			WHERE MONTH(date) = '$cur_month' AND YEAR(date) = '$cur_year' 
			GROUP BY WEEK(date) "; 
$db->setQuery($qry2);
$results2 = $db->loadObjectList();
$bridqry = "SELECT `zip`,`hits`,`date` FROM `#__jsplocation_ziphits` WHERE MONTH(date) = '$cur_month' AND YEAR(date) = '$cur_year' GROUP BY zip";
$db->setQuery($bridqry);
$brids = $db->loadObjectList();
$j=1;
$week[0][0] = "'".'Zip hits'."'";
...
foreach($brids as $bid){
	...
	$bhqry = "SELECT date,SUM(hits) as total_hits,
            ...
            DATE_ADD(date, INTERVAL(7-DAYOFWEEK(date)) DAY) as enddate
            FROM `#__jsplocation_ziphits`  
			WHERE MONTH(date) = '$cur_month' AND YEAR(date) = '$cur_year' AND zip = '$cur_id'
			GROUP BY WEEK(date) "; 
    $db->setQuery($bhqry);
...
?>
 <script type="text/javascript">
    ...
    <div id="infodiv2"><?php echo JText::_('WEEKLY_ZIP_DESC'); ?></div>
    <div id="chartdiv2" style="width: 1000px; height: 400px;"> </div>  <!-- the weekly graph container-->
...
$db->setQuery($qry2);
$results2 = $db->loadObjectList();
$bridqry = "SELECT `zip`,`hits`,`date` FROM `#__jsplocation_ziphits` GROUP BY zip "; 
$db->setQuery($bridqry);
$brids = $db->loadObjectList();
$j=1;
$monthlyhits[0][0] = "'".'Zip hits'."'";
foreach($brids as $bid){
    $monthlyhits[0][$j] = "'".$bid->zip."'";
	$j++;
}
...
?>
	...
	<div id="infodiv3"><?php echo JText::_('MONTHLY_ZIP_DESC'); ?></div>
     <div id="chartdiv3" style="width: 600px; height: 400px;"> </div>  <!-- the monthly graph container-->
...
<!-- display button -->
   ...
   function showDaily()
		  ...
		  function fullscreen()
		  { 
		     window.location="index.php?option=com_jsplocation&task=zipgraph&tmpl=component";
		}
		  function ret()
		  {
		     window.location="index.php?option=com_jsplocation&task=zipgraph";
		}
 showMonthly();
 </script>
