...
function display($tpl=null){
    ...
    if($dbprefix){
 			if($dbprefix == 'error'){ 
			$image = JURI::base().'components/com_jsecure/images/deleted.png';
			$dbprefix  = '<p class="dbprefixclass" style="color:red;font-weight:bold;"><img src='.$image.'> Database user has insufficient permissions to rename tables. Kindly check the user permissions in database to successfully change database prefix</p>';
			$dbprefix  .= '</div>';
	...
	function submitdbprefix(){
		...
		$checunderscore = substr($prefix, -1);	
        if($checunderscore!= "_"){
			$prefix = $prefix."_";
		}
