...
class jsecureViewEmailcheck extends JViewLegacy {
	protected $form;
	protected $item;
	protected $state;
	...
	{		
			...
			JToolBarHelper::apply('applyEmailcheck');
			JToolBarHelper::save('saveEmailcheck');
	...
	function save(){
 	...
 	function apply(){
		...
		$result = $this->saveDetails();
 		if($result){
 			$link = 'index.php?option=com_jsecure&task=emailcheck';
 			$msg  = 'Details has been saved';
 			$app->redirect($link,$msg,'MESSAGE');
 	    }
 	}
...
//		$modifiedFieldName	=$this->checkModifiedFieldName($config_array, $oldValue, $JSecureCommon, $keyvalue, $masterkeyvalue);
   ...
   function sendmail($JSecureConfig, $fieldName){
	...
	function emaillog($tpl=null){
       $mainframe = Jfactory::GetApplication();
        $model = $this->getModel('jsecurelog');
        $result = $model->getSpamEmail();
        $context = JRequest::getVar('option');
       $limit        = $mainframe->getUserStateFromRequest('global.list.limit', 'limit', $mainframe->getCfg('list_limit'), 'int');
	   $search  = $mainframe->getUserStateFromRequest( $context.'email'.'search','search','','string' );
       $search  = JString::strtolower($search);
        $limitstart    = $mainframe->getUserStateFromRequest($context.'limitstart', 'limitstart', 0, 'int');
        $limitstart = ( $limit != 0 ? (floor($limitstart / $limit) * $limit) : 0 );
        $totalcount = $model->getCountList();
	...
	JToolBarHelper::title(JText::_('jSecure Authentication'), 'generic.png');
    JToolBarHelper::cancel('cancelspamlog');
    JToolBarHelper::deleteList($msg = 'Are you sure want to delete?',$task = 'remove',$alt = 'Delete');
     ...
     $task = JRequest::getCmd('task');      
      ...
      //$logid=implode(',',$post['spamlog']);
	  $logid=implode(',',$post['cid']);
	  	  
      $model = $this->getModel('jsecurelog');
      $log= $model->removeEmaillog($logid);
     
     $link = 'index.php?option=com_jsecure&task=emaillog';
