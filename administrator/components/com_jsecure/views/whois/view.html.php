...
class jsecureViewWhois extends JViewLegacy {
	protected $form;
	protected $item;
	protected $state;
...
function display($tpl=null){
		...
		$this->assignRef('whoisinfo',$whoisinfo);
		//log end here
		parent::display($tpl);		
	}
	...
	function getwhoisinfo(){
		...
		if($_POST['domain_lookup'] != ""){
		$domain = strip_tags(stripslashes($_POST['domain_lookup']));
		$whoisinformation = $this->domain_whois($domain);
	...
	function domain_whois($domainname,$port=43){
		...
		$domainname = ltrim($domainname,'www.');
		...
		if($server == "")
		return "Invalid or an unsupported domain extension " . $domainname;	
