...
class JFormFieldCategoryEdit extends JFormFieldList
{
	...
	protected function getOptions()
	{
		...
		$query->select('a.id AS value, a.title AS text, a.level, a.published, a.lft, a.rgt');
		...
		if($catid)
		{
			$sql = 'SELECT * FROM `#__categories` WHERE `extension` LIKE "'. $this->element['extension'] . '" AND `id` LIKE '. $catid ;
			$db->setQuery($sql);
			$currentcat = $db->loadObjectList();
			$currentlft = $currentcat[0]->lft;
			$currentrgt = $currentcat[0]->rgt;
		}
		...
		// by "VPS" stores categories list in raw format
		$rawoptions = $db->loadObjectList();
		...
		if ($oldCat == 0)
		{
			foreach ($options as $i => $option)
			{
				...
				if($catid)
				{
					// this condition will hide the current related category
					if( ($options[$i]->lft > $currentlft) && ($options[$i]->rgt < $currentrgt) || ($options[$i]->lft == $currentlft) && ($options[$i]->rgt == $currentrgt) )
					{
						unset($options[$i]);
					} else {
						// then its okay
