...
class jspticketsModelpriority extends JModelAdmin {
	...
	public function getForm($data = array(), $loadData = true)
        {
			...
			// Get the form.
            $form = $this->loadForm('com_jsptickets.priority', 'priority', array('control' => 'jform', 'load_data' => true));
            if (empty($form)) {
                return false;
            }
            return $form;
	...
	{
		$db = JFactory::getDBO();
		$query = 'SELECT count(*) AS `count` FROM `#__jsptickets_priorities` AS a';
	...
	public function getList($listOrder=null, $listDirn=null, $filter_published=null, $search, $total)
	{
		...
		$query->from('#__jsptickets_priorities AS a');
	...
	{
		$db = JFactory::getDBO();
		$query = 'SELECT * FROM `#__jsptickets_priorities` WHERE `id` LIKE '. $id ;
		$db->setQuery($query);
		$data = $db->loadObjectList();
		return $data;
	}
	
	public function publishpriority($id)
	{
		$db = JFactory::getDBO();
		$query = "UPDATE `#__jsptickets_priorities` SET `publish` = '1' WHERE `id` = " . $id ;
		$db->setQuery($query);
		if (!$db->query()) 
		{
	...
	{
		$db = JFactory::getDBO();
		$query = "UPDATE `#__jsptickets_priorities` SET `publish` = '0' WHERE `id` = " . $id ;
		$db->setQuery($query);
		if (!$db->query()) 
		{
			JError::raiseError( 500, $db->getErrorMsg() );
	...
	{
		$db = JFactory::getDBO();
		$query = "DELETE FROM `#__jsptickets_priorities` WHERE `id` = " . $id ;
		$db->setQuery($query);
		if (!$db->query()) 
		{
			JError::raiseError( 500, $db->getErrorMsg() );
