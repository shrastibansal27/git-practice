   ...
   $result = $db->execute(); 
	 ...
	 function yahooApiData($id){
		 ...
		 $temp = array();
			...
			$area = $output->query->results->place->name;
			...
			$cityquery ="SELECT id,title FROM #__jsplocation_city where title ='".$city."'";
				...
				$areacheck ="SELECT id,title FROM #__jsplocation_city where title ='".$area."'";
				$db->setQuery($areacheck);
				$arearows = $db->loadObjectList();
				$areaid = $arearows[0]->id;
				$areaid = $arearows[0]->title;
			}
            ...
            if(($countryid !=''||$stateid !=''||$cityid !=''||$areaid !='')){
				...
				$query = "INSERT INTO #__jsplocation_branch (branch_name,address1,latitude,longitude,lat_long_override,zip,area_id,city_id,state_id,country_id,category_id,contact_person,gender,email,website,contact_number,description,facebook,twitter,published) VALUES('".$name."','".$addresslashdetail."','".$latitude."','".$longitude."','','','".$areaid."','".$cityid."','".$stateid."','".$countryid."','','','','','','','','','','1')";			
				$db->setQuery($query);
				$db->query($query);
