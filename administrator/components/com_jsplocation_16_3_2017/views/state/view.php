...
JToolBarHelper::title(JText::_('STATE_LIST'), 'state_article.png');
jimport( 'joomla.application.component.view');
require_once(JPATH_COMPONENT_ADMINISTRATOR.'/'.'tables'.'/'.'state.php');

class JspLocationViewState extends JViewLegacy {
	...
	function display($tpl = null) {
		...
		$search				= $mainframe->getUserStateFromRequest( $context.'state'.'search',			'search',			'',	'string' );
	...
	function form($tpl = null){
		...
		if($id!=0){
			$stateModel->id = $id;
			$data = $stateModel->getList();
			$this->assignRef("data",$data);
		} 
		else
		{
			$data =& JTable::getInstance('state');
			$this->assignRef("data",$data);
		}
		$countryList[]		= JHTML::_('select.option',  '0', JText::_( 'Select Country' ), 'id', 'title' );
		$countryList        = array_merge( $countryList, $stateModel->countryList());
		$list['country']    = JHTML::_('select.genericlist',$countryList, 'country_id', 'class="inputbox chzn-done" size="1"','id', 'title', $data->country_id );
	...
	function formadd($tpl = null){
		$stateModel     =& $this->getModel('state');
			...
			$countryList[]		= JHTML::_('select.option',  '0', JText::_( 'Select Country' ), 'id', 'title' );
		$countryList        = array_merge( $countryList, $stateModel->countryList());
		$list['country']    = JHTML::_('select.genericlist',$countryList, 'country_id', 'class="inputbox" size="1"','id', 'title', $data->country_id );
	...
	function saveData($tpl = null){
		...
		$row->title = JRequest::getVar('state','');
		...
		switch($task){
			case 'save':
			default:
				$link = "index.php?option=com_jsplocation&controller=state&task=state";
				$msg	= JText::_( 'State saved successfully' );
			break;
			...
			case 'apply':
				...
				$link = "index.php?option=com_jsplocation&controller=state&task=edit&cid[]=".$id;
	...
	function deleteData($tpl = null){
		...
		$link = "index.php?option=com_jsplocation&controller=state";
