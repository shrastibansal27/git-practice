...
class jspticketsViewtickets extends JViewLegacy
{
        ...
        function display($tpl = null) 
        {
			...
			$this->guestname=$this->guestemail=$this->ticketid="";
			
			// if($session->get("Itemid") != "")
			// {
				// $this->ItemId = $session->get("Itemid");
			// } else {
				// $this->ItemId = "";
			// }
			...
			{
				$this->ItemId	= $_COOKIE["cookie_itemid"];
			} else {
				$this->ItemId	= $session->get('Itemid');
			}
			
			//if registered user comes in and guest user details are still set
			if($user->id != 0 && $session->get( 'guestemail') != "" && $session->get( 'guestname') != "" )
			{
				$session->set( 'guestname', '' );
				$session->set( 'guestemail', '' );
				$session->set( 'ticketid', '' );
				$mainframe->redirect("index.php?option=com_jsptickets&Itemid=".$this->ItemId, "");
			}
			...
			if($user->id == 0) /* For Guest Users Only */
			{
				/* Condition for only guest search panel starts here */
				if($user->id == 0 && ( ( isset($post['jform']['guestemail']) && isset($post['jform']['ticketid']) && $post['jform']['guestemail'] != "" && $post['jform']['ticketid'] != "" ) || ($session->get("guestname") != "" && $session->get("guestemail") != "" && $session->get("ticketid") != "" ) ) )
				{					
					if(isset($post['jform']['guestemail']) && $post['jform']['guestemail']!="" && $user->id == 0)
					{
						$session->set( 'guestemail', $post['jform']['guestemail'] );
						$this->guestemail = $post['jform']['guestemail'];
					} else if( $session->get( 'guestemail') != "" && $user->id == 0 )	{
						$this->guestemail = $session->get( 'guestemail' );
					}
					
					if(isset($post['jform']['ticketid']) && $post['jform']['ticketid']!="" && $user->id == 0)
					{
						$session->set( 'ticketid', $post['jform']['ticketid'] );
						$this->ticketid = $post['jform']['ticketid'];
					} else if( $user->id == 0 )	{
						$this->ticketid = $session->get( 'ticketid' );
					}
					
					if($this->ticketid != "" && $this->guestemail != "" && $user->id == 0)
					{
						$this->guestname = $model->getGuestName($this->guestemail, $this->ticketid); // get the Guest Name from Guest Email & Ticket Id
						$session->set( 'guestname', $this->guestname );
					} else if( $session->get( 'guestname' ) != "" && $user->id == 0 )	{
						$this->guestname = $session->get( 'guestname' );
					}
					
					$checkempty = $model->getCheckEmptyList( $this->guestname, $this->guestemail, $this->ticketid );
					
					if( $user->id == 0 && isset($this->ticketid) && $this->ticketid != "")
					{
						$realid = $model->get_Ticketid_from_coded($this->ticketid);
						$Bool_Val = $this->Throw_Error($realid, $checkempty);
						if($Bool_Val == 1 && $this->ticketid != "")  // Don't throw message until the new ticket form is opened cause in that case ticketid be "null"
						{
				...
				/* Condition for only guest search panel ends here */
				
				/* Condition for only guest new ticket panel starts here */
				if($user->id == 0 && (isset($post['jform']['new_guestname']) && isset($post['jform']['new_guestemail']) && $post['jform']['new_guestname'] != "" && $post['jform']['new_guestemail'] != "") )
				{
					if(isset($post['jform']['new_guestname']) && $post['jform']['new_guestname'] != "") // we have to check both if the Guest is coming from login page or after editing
					{
						$session->set( 'guestname', $post['jform']['new_guestname'] );
						$this->guestname = $post['jform']['new_guestname'];
					}
					
					if(isset($post['jform']['new_guestemail']) && $post['jform']['new_guestemail'] != "")
					{
						$session->set( 'guestemail', $post['jform']['new_guestemail'] );
						$this->guestemail = $post['jform']['new_guestemail'];
					}
					$session->set( 'ticketid', "" );
					$this->ticketid = "" ;
				}
				/* Condition for only guest new ticket panel ends here */
				
				if( ( $this->guestname != "" && $this->guestemail != "" && $user->id == 0 && ($checkempty == "" || $this->ticketid == "") ) || ($user->id != 0 && $checkempty == "") ) // if there is no ticket for the user then redirect him directly to add ticket form
				{
			...
			$this->fltr_count = $model->fltrCount( $this->listOrder, $this->listDirn, $this->search, $this->filterType, $this->filterCat, $this->filterFollow_up, $this->filterPriority, $this->filterStatus, $this->filterAssigned_to, $this->guestname, $this->guestemail );
			$limit		= $mainframe->getUserStateFromRequest('global.list.limit', 'limit', $mainframe->getCfg('list_limit'), 'int');
			$limitstart	= $mainframe->getUserStateFromRequest($context.'limitstart', 'limitstart', 0, 'int');
			$total_tickets = count($model->gettotaltickets($this->guestname, $this->guestemail, $this->ticketid));
			...
			{
				$total = count($this->fltr_count); // gets count for list with ACL and also search filters
			} else { 
				$total = $total_tickets; // gets count only for list with ACL and no search filters
			}
			
			$this->data = $model->getList( $this->listOrder, $this->listDirn, $this->search, $this->filterType, $this->filterCat, $this->filterFollow_up, $this->filterPriority, $this->filterStatus, $this->filterAssigned_to, $this->guestname, $this->guestemail, $total );
		...
		function form($tpl = null)
		{
			...
			if( $session->get( 'guestname') != "" && $user->id == 0)
			{
				$this->ticketguestname = $session->get( 'guestname');
			} else {
				$this->ticketguestname = '';
			}
			
			if( $session->get( 'guestemail') != "" && $user->id == 0)
			{
				$this->ticketguestemail = $session->get( 'guestemail' );
			} else	{
				$this->ticketguestemail = '';
			}
			
			if( $session->get( 'ticketid') != "" && $user->id == 0)
			{
				$this->ticketguestticketid = $session->get( 'ticketid' );
			} else	{
				$this->ticketguestticketid = '';
			}
			...
			$this->config = $config;
			$this->key = JRequest::getVar("key");         /* Don't Touch */
			$this->pagination = new JPagination($total, $limitstart, $limit);
		...
		function saveData()
		{
			...
			if(isset($post['jform']['created_for']))
			{
				$created_for = $post['jform']['created_for'];
			} else if($post['created_for'] != "")
			{
				$created_for = $post['created_for'];
			} else {
				$created_for = 0;
			}
			
			if(isset($post['jform']['assigned_to']))
			{
				$assigned_to = $post['jform']['assigned_to'];
			} else if($post['assigned_to'] != "")
			{
			...
			} else {                                           // if categories are sent blank then by default uncategorised category of com_jsptickets will be set
				$extarr[$f++] = "com_jsptickets";
				$post['category_id'] = $this->selectDefaultCat('com_jsptickets');
				$category = json_encode($post['category_id']);
			}
			
			$extstr = json_encode($extarr); // will be used in saving category extension
			...
			if($post['description'] == null)  //if ticket description is null
				$mainframe->redirect(JRoute::_("index.php?option=com_jsptickets&view=tickets&controller=ticketlist&task=edit&ticketcode=".$ticketcode) , JText::_("TICKET_DESCRIPTION_EMPTY"), "ERROR");
			...
			{   //check file extension for the uploaded file
				if (  in_array( strtolower($imageext), $allowedext ) ) {
					...
					if( $filesize > 25000000 ){
						$mainframe->redirect( JRoute::_("index.php?option=com_jsptickets&view=tickets&controller=ticketlist&task=edit&ticketcode=".$ticketcode) , JText::_("FILESIZE_GREATER_THAN_25MB"), "ERROR" );
					} else {
						if ( JFile::upload($src, $dest) ) {
							//Redirect to a page of your choice
						} else {
							//Redirect and throw an error message
							$mainframe->redirect( JRoute::_("index.php?option=com_jsptickets&view=tickets&controller=ticketlist&task=edit&ticketcode=".$ticketcode) , JText::_("FILE_COUNDNOT_BE_UPLOADED"), "ERROR" );
						}
					}
				} else {
					$mainframe->redirect( JRoute::_("index.php?option=com_jsptickets&view=tickets&controller=ticketlist&task=edit&ticketcode=".$ticketcode) , JText::_("FILE_EXTENSION_NOT_ALLOWED"), "ERROR" );
				}
			}
			
			$ticketcodedid =  uniqid("", false);    // later will be used in new tickets
			
			/* if($user->id == 0) // if a guest user
			{
					if($id)
					{
						$session->set( 'ticketid', $post['ticketcode'] ); // update guest session with edited ticket id 
					}
					else
					{
						$session->set( 'ticketid', $ticketcodedid ); // update guest session with latest ticket id 
					}
			} */
			...
			{	
				...
				if($config[0]->email_notification == 1) 
				{
					...
					{
						$assignedtomail = $this->SendMailFunction("assignedto_changed", $id, $post['ticketcode'], $category, $post['jform']['title'], $post['comments'], $post['jform']['subject'], $post['description'], $user->id, $created_for, $assigned_to,  $post['guestname'], $post['guestemail'], $config[0]->new_mail_subject, $config[0]->new_mail_body);
					}
					if($coid != '' and $email_comment != 0 )
					{
						$commentmail = $this->SendMailFunction("newcomment", $id, $post['ticketcode'], $category, $post['jform']['title'], $post['comments'], $post['jform']['subject'], $post['description'], $user->id, $created_for, $assigned_to,  $post['guestname'], $post['guestemail'], $config[0]->new_mail_subject, $config[0]->new_mail_body);
					}
				}
				...
				if( $post['jform']['status'] == 2 )
					$query = 'INSERT INTO `#__jsptickets_ticket` (`ticketcode`, `title`, `subject`, `description`, `category_extension`, `category_id`, `priority_id`, `status_id`, `attachment_id`, `feedback_id`, `comment_id`, `created`, `created_by`, `created_for`, `modified`, `assigned_by`, `assigned_to`, `closed`, `closed_by`, `email_comment`, `guestname`, `guestemail`) VALUES ("'. $ticketcodedid .'", "' . $this->escape_string($post['jform']['title']) . '", "' . $this->escape_string($post['jform']['subject']). '", "' . $post['description']. '", ' . "'". $extstr . "', '" . $category . "'" . ', ' . $post['jform']['priority'] . ', ' . $post['jform']['status'] . ', "' . $naid . '", "' . $nfid . '", "' . $ncid . '", "' . $currtimestamp . '", ' . $user->id . ", " . $created_for . ', "' . $currtimestamp . '", ' . $user->id . ", " . $assigned_to . ", " . '"' . $currtimestamp . '", ' . $user->id . ', "' . $email_comment . '", "'. $post['guestname'] .'", "' . $post['guestemail'] . '");';
				else
					$query = 'INSERT INTO `#__jsptickets_ticket` (`ticketcode`, `title`, `subject`, `description`, `category_extension`, `category_id`, `priority_id`, `status_id`, `attachment_id`, `feedback_id`, `comment_id`, `created`, `created_by`, `created_for`, `modified`, `assigned_by`, `assigned_to`, `email_comment`, `guestname`, `guestemail`) VALUES ("'. $ticketcodedid .'", "' . $this->escape_string($post['jform']['title']) . '", "' . $this->escape_string($post['jform']['subject']). '", "' . $post['description'] . '", ' . "'". $extstr . "', '" . $category . "'" .', ' . $post['jform']['priority'] . ', ' . $post['jform']['status'] . ', "' . $naid . '", "' . $nfid . '", "' . $ncid . '", "' . $currtimestamp . '", ' . $user->id . ", " . $created_for .  ', "' . $currtimestamp . '", ' . $user->id . ", " . $assigned_to . ', "' . $email_comment . '", "'. $post['guestname'] .'", "' . $post['guestemail'] . '");';
				$db->setQuery($query);
				if (!$db->query()) 
				{
					JError::raiseError( 500, $db->getErrorMsg() );
				...
				if( $config[0]->email_notification == 1 )
				{
					$sendmail = $this->SendMailFunction("new" , $id, $ticketcode, $category, '', '', $post['jform']['subject'], $post['description'], $user->id, $created_for, $assigned_to,  $post['guestname'], $post['guestemail'], $config[0]->new_mail_subject, $config[0]->new_mail_body);
					
					// if new comment is added at any condition and comment is to be sent through mail
					if($coid != '' and $email_comment != 0) 
					{
						$commentmail = $this->SendMailFunction("newcomment", $id, $ticketcode, $category, $post['jform']['title'], $post['comments'], $post['jform']['subject'], $post['description'], $user->id, $created_for, $assigned_to,  $post['guestname'], $post['guestemail'], $config[0]->new_mail_subject, $config[0]->new_mail_body);
					}
				}				
			...
			if($session->get("ticketid") == "" && $user->id == 0) // if a new ticket is to be opened
			{
				$guestlink = 'index.php?option=com_jsptickets&view=tickets&controller=ticketlist&task=cancel&Itemid='.$ItemId;
				$newticketcondn = 1;
			} else {
				$newticketcondn = 0;
			}
			switch($task)
			{
				...
				case 'default':
					...
					$msg = $config[0]->ticket_saved_msg;
					if($newticketcondn == 1)
					{
						$mainframe->redirect(JRoute::_( $guestlink ), $msg, "MESSAGE");
					} else {
						$mainframe->redirect(JRoute::_('index.php?option=com_jsptickets&view=tickets&task=ticketlist&Itemid='.$ItemId), $msg, "MESSAGE");
					}
					break;
				...
				case 'apply':
					...
					if($newticketcondn == 1)
					{
						...
						$mainframe->redirect(JRoute::_('index.php?option=com_jsptickets&view=tickets&controller=ticketlist&task=edit&ticketcode='. $ticketcode.'&Itemid='.$ItemId), $msg, "MESSAGE");
					}
					break;
		    }
		}
		
		function Throw_Error($itemId = "", $itemList = "")
		{
			$throw_exception = 1;     // 1 if error exists
			if( $itemList != "" )
			{
				foreach($itemList AS $itemArray)
				{
					if($itemId == $itemArray->id)
					{
						$throw_exception = 0;   // 0 if error doesnot exists
					}
				}
			}
		...
		function follow()
		{
			...
			$msg = $config[0]->ticket_followup_msg;
			$mainframe->redirect( JRoute::_("index.php?option=com_jsptickets&view=tickets&task=ticketlist&Itemid=".$ItemId) , $msg, "MESSAGE" );
		}		
		...
		{ 
			...
			$msg = $config[0]->ticket_removefollow_msg;
			$mainframe->redirect( JRoute::_("index.php?option=com_jsptickets&view=tickets&task=ticketlist&Itemid=".$ItemId) , $msg, "MESSAGE" );
		}		
		...
		function remove()
		{
			...
			$msg = $config[0]->ticket_removed_msg;
			$mainframe->redirect( JRoute::_("index.php?option=com_jsptickets&view=tickets&task=ticketlist&Itemid=".$ItemId) , $msg, "MESSAGE" );
		}
		
		function selectDefaultCat($ext = null)
		{
			$db = JFactory::getDBO();
			$query = 'SELECT `id` FROM `#__categories` WHERE `alias` LIKE "uncategorised" AND `extension` LIKE "'. $ext .'"' ;
			$db->setQuery($query);
			$data = $db->loadObjectList();
			return (array)$data[0]->id;
		}
		
		function SendMailFunction($mtask = null, $id = null, $ticketcode = null, $category = null, $ticket_title = null, $comment = null, $ticket_sub = null, $ticket_desc = null, $created_by = 0, $created_for = 0, $assigned_to = 0, $guestname = null, $guestemail = null, $configuser_mail_subject = null, $configuser_mail_body = null)
		{
