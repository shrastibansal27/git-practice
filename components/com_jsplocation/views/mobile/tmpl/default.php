...
<form method="post" name="adminForm" id="adminForm" class="form-validate">
  ...
  <div class="jsp_map_block">
	...
	<div class="jsp_wrap">
		...
		<div class="locate">
            	<div class="loc">
		<a href="#" style="color:#fff !important;" onclick="return getGeoloc();" title="<?php echo JText::_('LOCATE_ME_DESC'); ?>">Locate me</a><img class="locate_not" src="<?php echo JURI::base();?>components/com_jsplocation/images/locate.png"><span><i class="fa fa-angle-right"></i></span>				
				</div>
		</div>			
          <?php
						}
			...
			function fromhere()
                   ...
                   {	
                    ...
                    function createTabbedMarker(pinstr, pointertype, fillcolor, fontsize, branch_id_disp, point, contactDetails, branchDirection, branchAddress, title, description, pointerImage ,label1, label2,branchimage,style)
                    {
							...
							{											
                            ?>                             
							var link = "index.php?task=redirectviewinfo_mobile&id="+id;
							if(branchimage){
							side_bar_html += '<li id="locationli" style="cursor:pointer" onclick="javascript:myclick(' + (overlays.length-1) + ');" title="<?php echo JText::_('LOCATION_LIST_TITLE');?>"><a id="brnotify" class="jsp_loc_branchdetails branch_notification" href="'+link+'" id="'+title.replace(/[" "]/g,"-")+'" title="<?php echo JText::_('LOCATION_LIST_TITLE');?>">' + branchAddress + '<\/a><div class="imgwrap"><img style=""  class="map_inner img" width="80px" height="80px" src="images/jsplocationimages/jsplocationImages/'+title+'/'+branchimage+'"'+'></div>';
                             }else{
							  side_bar_html += '<li id="locationli" style="cursor:pointer" onclick="javascript:myclick(' + (overlays.length-1) + ');" title="<?php echo JText::_('LOCATION_LIST_TITLE');?>"><a id="brnotify" class="jsp_loc_branchdetails branch_notification" href="'+link+'" id="'+title.replace(/[" "]/g,"-")+'" title="<?php echo JText::_('LOCATION_LIST_TITLE');?>">' + branchAddress + '<\/a>';
							}
                            <?php }
